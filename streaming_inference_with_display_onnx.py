...
def extract_mel_combined_window(input):
    S = librosa.feature.melspectrogram(y=input, sr=16000, n_fft=1024, hop_length=160, n_mels=256)
    S_DB = librosa.power_to_db(S, ref=np.max)
    S_normalized = (S_DB - np.min(S_DB)) / (np.max(S_DB) - np.min(S_DB) + 1e-8)
    S_resized = cv2.resize(S_normalized, dsize=(256, 200), interpolation=cv2.INTER_LINEAR)
    return S_resized
...
def process_window(session, window, threshold):
    S = librosa.feature.melspectrogram(y=window, sr=16000, n_fft=1024, hop_length=160, n_mels=256)
    S_DB = librosa.power_to_db(S, ref=np.max)
    S_normalized = (S_DB - np.min(S_DB)) / (np.max(S_DB) - np.min(S_DB) + 1e-8)
...
def audio_callback(indata, frames, time_info, status):
    ...
    while len(buffer) >= window_size:
        ...
        print(f"                 Mobilenet_v3     {output_probability:.3f}   {wakeword_status}")
        ...
        if output_probability > threshold:
            ...
            if flag == 1:
                ...
                if Total_prob > 0.5:
                    ...
                    print("                     |___ Combined window probability: {:.3f} ---- {} detected! Total count: {}".format(Total_prob, "WAKE WORD", wake_word_count+1))
                else:
                    # Clear and update the branch line
                    print("\033[8;0H\033[K", end='')  # Move cursor to row 8 and clear line
                    print("                     |")
                    # Clear and update the combined probability line
                    print("\033[9;0H\033[K", end='')  # Move cursor to row 9 and clear line
                    print("                     |___ Combined window probability: {:.3f} ---- {} detected! Total count: {}".format(Total_prob, "NOT", wake_word_count))
